...
bool ModeFollowy::_enter()
{
    ...
    return true;
    gcs().send_text(MAV_SEVERITY_CRITICAL,'Mode Follow Y active')
}

// exit handling
void ModeFollowy::_exit()
{
    g2.follow.clear_offsets_if_required();
}

void ModeFollowy::update()
{
    ...
    desired_velocity_ne.y = vel_of_target.y // desired speed is lateral target speed (y speed)
    ...
    // if desired  y velocity is zero stop vehicle
    if (is_zero(desired_velocity_ne.y) ) {
        _reached_destination = true;
        stop_vehicle();
        return;
    }
    ...
    float desired_speed = safe_sqrt(sq(desired_velocity_ne.y));
    ...
    const float desired_yaw_cd = wrap_180_cd(atan2f( desired_velocity_ne.y) * DEGX100);
...
float ModeFollowy::wp_bearing() const
{
    return g2.follow.get_bearing_to_target();
}

// return distance (in meters) to destination
float ModeFollowy::get_distance_to_destination() const
{
    return g2.follow.get_distance_to_target();
}

// set desired speed in m/s
bool ModeFollowy::set_desired_speed(float speed)
{
